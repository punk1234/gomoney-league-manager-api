...
describe("DELETE /fixtures/:fixtureId", () => {
  beforeAll(async () => {
  ...
  it("[200] - Delete fixture with valid data", async () => {
    const res = await request(app)
      .delete(`/fixtures/${createdFixture._id}`)
  ...
  it("[400] - Delete fixture with invalid ID format in request object", async () => {
    const res = await request(app).delete("/fixtures/abcd").expect(C.HttpStatusCode.BAD_REQUEST);

    expect(res.body).toHaveProperty("message");
  });

  it("[401] - Delete fixture without auth-token", async () => {
    const res = await request(app)
      .delete(`/fixtures/${createdFixture._id}`)
  ...
  it("[403] - Delete fixture without being an admin", async () => {
    const res = await request(app)
      .delete(`/fixtures/${createdFixture._id}`)
  ...
  it("[404] - Delete fixture with ID that does not exist", async () => {
    const res = await request(app)
      .delete("/fixtures/abcd1234abcd1234abcd1234")
      .set({ authorization: `Bearer ${adminLoginInfo.token}`, "Content-Type": "application/json" })
