...
describe("POST /auth/login", () => {
    ...
    it("[401] - Login user with email that does not exist (confirm 404 is not returned for security concern)", async () => {
        const res = await request(app)
            .post("/auth/login")
    ...
    it("[429] - Logging in severals times with failed attempts", async () => {
        const MAX_NO_OF_REQUEST_PLUS_ONE = config.API_RATE_LIMITING[C.ApiRateLimiterType.AUTH_LOGIN].limit + 1;

        const email = "login-rate-limit-email@rate.limit";
        const password = "p@ssword";

        const USER = await USER_SERVICE.createUser({ email, password, isAdmin: false });
